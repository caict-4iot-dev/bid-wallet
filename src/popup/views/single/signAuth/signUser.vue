    ...
    <div class="main-container container-no-header">
      ...
      <div class="main-container-wrapper">
        ...
        <div class="sub-btn-group clearfix" style="margin-top: 120px;">
          ...
          <van-button type="info" class="btn-info fr" @click="toLoginAuth">{{$t('app.btnText.next')}}</van-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    toLoginAuth () {
      this.$router.push({path: '/signAuthConnect',
        query: {
          url: this.websiteInfo.url,
          icon: this.websiteInfo.icon,
          uuid: this.websiteInfo.uuid,
          tabId: this.websiteInfo.tabId,
          randomCode: this.websiteInfo.randomCode,
          type: MessageTypes.SIGN_AUTH
        }})
    },
    cancel () {
    ...
    sendMsg (code, fn) {
      ...
      bewExtension.sendTabMessage(this.websiteInfo.tabId, {type: MessageTypes.SIGN_AUTH, ...resultMsg})
      if (fn) fn()
    },
    changeUserFn () {
