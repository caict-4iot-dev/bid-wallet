...
export default {
  ...
  methods: {
    ...
    toLoginAuth () {
      this.$router.push({path: '/signMessageConnect',
        query: {
    ...
    sendMsg (code, fn) {
      ...
      bewExtension.sendTabMessage(this.websiteInfo.tabId, {type: MessageTypes.SIGN_MESSAGE, ...resultMsg})
      if (fn) fn()
    },
    changeUserFn () {
  ...
  mounted () {
    document.title = this.$t('app.sign.signAccount')
